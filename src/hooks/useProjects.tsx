...
const useProjects = () => {
    ...
    const getProjects = async () => {        
        setIsLoading(true)
        const result = await getProjectsFromDb(query(projectsCollectionRef, limit(6), orderBy("name")))
        setProjects(result)
        setIsLoading(false)
      }
      ...
      const fetchNextPage = async () => {
        ...
        const next = query(projectsCollectionRef, limit(6), orderBy("name"), startAfter(lastVisible));        
